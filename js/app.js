...
btn.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch(
      "https://vuelos-bfs.onrender.com/api/buscar/",
      {
        method: "POST",
    ...
    if (response.ok && data.ruta) {
      const rutaFormateada = data.ruta.join(
    ...
    mostrarError(
      "Error de conexión. Verifica que el servidor Django en Arch esté activo.",
    );
  } finally {
    btn.disabled = false;
...
async function cargarCiudades() {
  try {
    const response = await fetch(
      "https://vuelos-bfs.onrender.com/api/ciudades/",
    );
    const data = await response.json();
    ...
    data.ciudades.forEach((ciudad) => {
      ...
      span.style.cssText =
        "background: #e2e8f0; padding: 2px 8px; border-radius: 4px; cursor: pointer;";
